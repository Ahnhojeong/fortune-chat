...
function IntroForture() {
  ...
  const startFortune = () => {
    if (!dateRef.current.value || !hourRef.current.value) {
      alert("생년월일 또는 시간을 입력해 주세요");
      return;
    }
    ...
    setMyDateTime(dateRef.current.value + hourRef.current.value);
  };
  ...
  return (
    <div className={layoutStyles.chat_container}>
      <div className={layoutStyles.chat_title}>
        ...
        <p className={layoutStyles.intro_message}>"자네.. 운세 한번 볼텐가?"</p>
      </div>
